...
class DatasetManager {
    constructor() {
        // Mengarah ke file CSV dataset pedoman akademik Anda
    ...
    getAllDocuments() {
        try {
            ...
            // 1. Baca berkas CSV menggunakan library xlsx
            const workbook = xlsx.readFile(this.csvFilePath);
            const sheetName = workbook.SheetNames[0];
            const worksheet = workbook.Sheets[sheetName];

            // 2. Konversi worksheet menjadi JSON Array objek
            const rawData = xlsx.utils.sheet_to_json(worksheet);
            const formattedDocs = [];

            // 3. Iterasi dan bersihkan setiap baris pedoman akademik
            rawData.forEach((row, index) => {
                ...
                // Pemotongan aman per 800 karakter agar tidak melebihi batas token RAG
                    ...
                    if (chunkText.length > 20) {
                        formattedDocs.push({
                            ...
                            metadata: {
                                ...
                                topik: currentTopic,
                                sumber: "Pedoman Academic TUS 2024"
                            }
                        });
                    }
                    start = end + 1;
